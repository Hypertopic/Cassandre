function(head, req) {
  ...
  while (row = getRow()) {
    ...
    if (row.value.modified_type) {
      ...
      if (row.doc.type) object.modified_type = row.doc.type;
    }
    ...
    if (row.value.comment) {
      ...
      if (row.doc.readers == undefined || row.doc.readers.length == 0 || users.indexOf(req.userCtx.name) > -1) data.activity.push(object);
    } else {
      if (row.value.users.indexOf(req.userCtx.name) > -1) data.activity.push(object);
    }
  }
