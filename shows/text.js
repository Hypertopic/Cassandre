function(doc, req){
  ...
  send('  var words = [];');
  send('  for (m=0; m<markups.length; m+=2) {\n');
  send('    words[m/2] = {text: markups[m].textContent.toLowerCase()};\n');
  send('  }\n');
  send('  words[0].count = 0;\n');
  send('  words[1].count = 0;\n');
  send('  var max = 0;\n');
  send('  for (w=0; w<words.length-2; w++) {\n');
  send('    var key =');
  send('      new Array(words[w].text, words[w+1].text, words[w+2].text);\n');
  send('    var nb = trigrams[key];\n');
  send('    if (!nb) nb = 1;');
  send('    words[w].count = Math.max(words[w].count, nb);\n');
  send('    words[w+1].count = Math.max(words[w+1].count, nb);\n');
  send('    words[w+2].count = nb;\n');
  send('    max = Math.max(max, words[w].count);\n');
  send('  }\n');
  send('  for (m=0; m<markups.length; m+=2) {');
  send('    markups[m].color = toColor(words[m/2].count/max);');
  send('  }');
  send('}\n');
